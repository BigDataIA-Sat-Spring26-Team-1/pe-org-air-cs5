...
class PatentCollector:
    ...
    Replaces the BigQuery implementation to focus on US Patents with higher precision.
    """
    ...
    def __init__(self, project_id: Optional[str] = None):
        # We accept project_id to maintain compatibility with existing orchestrator, 
        # but we use PatentsView API Key from env instead.
        self.output_file = "patent_signals.csv"
        api_key = os.getenv("PATENTSVIEW_API_KEY", "").strip()
        if not api_key:
            logger.warning("Missing PATENTSVIEW_API_KEY env var. PatentCollector will likely fail.")
    ...
    async def collect(self, company_name: str) -> CollectorResult:
        ...
        metadata = {
            ...
            "ai_count": len(ai_patents), # Backward compatibility
            "total_filings": len(patents), # Total patents found (AI + non-AI)
            "representative_patents": [
                ...
                for p in ai_patents[:10]  # First 10 as examples
            ]
        }
        
        raw_val = f"{len(ai_patents)} AI patents identified"
        if not ai_patents and patents:
            raw_val = f"No AI patents found out of {len(patents)} total filings"
        elif not patents:
            raw_val = "No recent patents found"
